...
install_awg_packages() {
    ...
    BASE_URL="https://github.com/KeiRun/awg-openwrt/releases/download/"

    # В этом форке собираются пакеты AmneziaWG 3.1
    AWG_VERSION="3.1"
...
configure_amneziawg_interface() {
    ...
    # AWG 2.0/3.1 дополнительные параметры обфускации
    if [ "$AWG_VERSION" = "2.0" ] || [ "$AWG_VERSION" = "3.1" ]; then
    ...
    # Устанавливаем параметры для AWG 2.0/3.1 (только если они заданы)
    if [ "$AWG_VERSION" = "2.0" ] || [ "$AWG_VERSION" = "3.1" ]; then
